import { CelebrateEmail } from '@/components/CelebrateEmail';
...
export default async function Home() {
  ...
  return <main id="contenido" className="site-main">
    <section className="hero scene" aria-labelledby="hero-title"><div className="glass-panel hero-glass"><LiquidText className="hero-liquid"><h1 id="hero-title"><ColorableName className="hero-name">{sitio.nombre}</ColorableName></h1><p>{sitio.hero}</p></LiquidText></div></section>
    <MarqueeBanner />
    ...
    <section className="scene" id="contacto" aria-labelledby="contacto-title"><div className="glass-panel section-glass"><ContactCelebration><h2 id="contacto-title">Contacto</h2></ContactCelebration><div className="contact-list">{sitio.canales.map((canal) => <a className="contact-row" href={canal.url} key={canal.url}><span className="meta">{canal.etiqueta}</span><span>{canal.etiqueta === 'Correo' ? <CelebrateEmail>{canal.valor}</CelebrateEmail> : canal.valor}</span><span aria-hidden="true">→</span></a>)}</div></div></section>
  </main>;
}
